...
export default function EventDetails({ event, isOpen, onClose }: Readonly<EventDetailsProps>) {
  ...
  const content = (
    <div className="flex flex-col h-full">
      <div className="relative h-[33vh] md:h-[40vh] w-full">
        <Image src={event.media.url} alt={event.name} fill className="object-cover" />
        ...
        <div className="absolute bottom-4 left-4 right-4 z-10">
          ...
          <p className="text-white/90 text-sm md:text-base">{event.event_progress.replace("_", " ")}</p>
        </div>
      </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{event.event_progress.replace("_", " ")}</p>
                  </div>
                  <div>
                    <p className="text-gray-500">Category</p>
                    <p className="font-medium text-gray-900">{event.categories.length}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <Link href={`/events/${event.id}/vote`} className="w-full">
                  <Button variant="secondary" className="w-full group" disabled={event.event_progress === "not_started"}>
                    Vote Now
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetDescription className=" sr-only">{event.description}</SheetDescription>
